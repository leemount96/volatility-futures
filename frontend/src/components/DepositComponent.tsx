...
export const DepositComponent = () => {
  ...
  const depositHandler = async (event: any) => {
    ...
    marginpool.functions.depositCollateral(depositAmount);
    GetCollateral();
  }
  ...
  if (collateralAmount.amount > 0) {
    depositCard = (
      <Card style={{ width: "18rem" }} className="me-5 mt-5">
        <Card.Body>
          <Card.Title>Withdraw Collateral</Card.Title>
          <Card.Text>Window for withdrawing collateral from margin pool</Card.Text>
        </Card.Body>
        ...
        <Card.Body>
          <Button variant="danger" onClick={withdrawHandler}>Withdraw Collateral</Button>
        </Card.Body>
      </Card>
    );
  } else {
    depositCard = (
      <Card style={{ width: "18rem" }} className="me-5 mt-5">
        ...
        <ListGroup className="list-group-flush">
          <ListGroupItem>Available USDC Balance:
                {walletUSDC.amount}
               </ListGroupItem>
        </ListGroup>
        <Card.Body>
          ...
          <InputGroup className="mb-3">
              <form onSubmit={depositHandler}>
                  <input id='depositAmount' type='text' placeholder="Amount of USDC"/>
