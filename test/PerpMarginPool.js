...
describe("PerpMarginPool", function () {
  ...
  beforeEach(async function () {
    ...
    deployedMarginPool = await PerpMarginPool.deploy(
      initMarginRate,
      lowRiskMargin,
      highRiskMargin,
      liquidationLevel,
      deployedOracle.address,
      deployedVPool.address,
      deployedUSDC.address
    );

    await deployedUSDC
      .connect(addr1)
      .approve(deployedMarginPool.address, addr1USDC);
    await deployedUSDC
      .connect(addr2)
      .approve(deployedMarginPool.address, addr2USDC);
    await deployedUSDC
      .connect(addr3)
      .approve(deployedMarginPool.address, addr3USDC);
  ...
  describe("Pool Deployment", function () {
    it("Deploying margin pool should set the correct initial variables", async function () {
      ...
      expect(await deployedMarginPool.marginLowRiskLevel()).to.equal(
        lowRiskMargin
      );
      expect(await deployedMarginPool.marginHighRiskLevel()).to.equal(
        highRiskMargin
      );
      expect(await deployedMarginPool.marginLiquidationLevel()).to.equal(
        liquidationLevel
      );
    });
  });
  ...
  describe("Collateral deposit/removal", function () {
    it("Depositing collateral should transfer USDC to contract and establish position in maps", async function () {
      ...
      expect(await deployedUSDC.balanceOf(deployedMarginPool.address)).to.equal(
        5000
      );
      expect(
        await deployedMarginPool.freeCollateralMap(addr1.address)
      ).to.equal(5000);
      ...
      expect(
        await deployedMarginPool.freeCollateralMap(addr2.address)
      ).to.equal(5000);
    });
    ...
    it("Attempting to withdraw more than deposited shoudl fail", async function () {
      ...
      await expect(
        deployedMarginPool.connect(addr1).returnCollateral(10000)
      ).to.be.revertedWith("Not enough free collateral");
    });
  });
  ...
  describe("Providing Liquidity via MarginPool contract", function () {
    ...
    it("Provide liquidity from addr1", async function () {
      ...
      expect(await deployedUSDC.balanceOf(deployedVPool.address)).to.equal(
        1000
      );
  ...
  describe("Opening new long/short positions", function () {
    ...
    it("Opening a new long position for user should create position and adjust free collateral", async function () {
      ...
      expect(await deployedUSDC.balanceOf(deployedMarginPool.address)).to.equal(
        8500
      ); //only transfer the actual amount to vpool, rest held in margin
      expect(await deployedUSDC.balanceOf(deployedVPool.address)).to.equal(
        1500
      );
      ...
      expect(await deployedMarginPool.liquidationRisk(addr2.address)).to.equal(
        false
      );
    });
    ...
    it("Opening a new short position for user should create position and adjust free collateral", async function () {
      ...
      expect(await deployedUSDC.balanceOf(deployedMarginPool.address)).to.equal(
        9500
      ); //USDC transfered from Vpool to margin
    ...
    it("Attempting to open position when one is already open should fail", async function () {
      await deployedMarginPool.connect(addr2).openLongPosition(500);
      await expect(
        deployedMarginPool.connect(addr2).openLongPosition(100)
      ).to.be.revertedWith("Already have open position");
      await expect(
        deployedMarginPool.connect(addr2).openShortPosition(100)
      ).to.be.revertedWith("Already have open position");
    });
  });
  ...
  describe("Closing open positions", function () {
    ...
    it("Open a long position and close it", async function () {
      ...
      expect(
        await deployedMarginPool.freeCollateralMap(addr2.address)
      ).to.equal(5034); //should be 5000, rounding
      expect(await deployedUSDC.balanceOf(deployedMarginPool.address)).to.equal(
        9010
      ); //rounding issue
